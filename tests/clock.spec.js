...
describe('<Clock />', () => {
  ...
  it('displays 60 by default', () => {
    expect(component.find('text').text()).toEqual('60');
  });
  ...
  it('decrements when the game starts', () => {
    ...
    clock.tick(1010);
    expect(component.find('text').text()).toEqual('59');

    clock.tick(1010);
    expect(component.find('text').text()).toEqual('58');
  });

  it('shows dial running when the game starts', () => {
    actions.startGame();

    expect(component.find('.running').length).toEqual(1);
  });
